...
public sealed partial class DisplayTint : IDisplayTint
{
    ...
    private readonly List<string> _outputs = ConnectedOutputs();
    ...
    {
        var blue = Blue[Math.Clamp(level, 0, Blue.Length - 1)];
        var g = "1:1:" + blue.ToString(CultureInfo.InvariantCulture);
        var ok = _outputs.Count > 0;
        return _outputs.Aggregate(ok, (current, o) => current & Run("--output", o, "--gamma", g).code == 0);
    }
    ...
    private static List<string> ConnectedOutputs()
    {
        ...
        foreach (Match m in ConnectedRegex().Matches(outp))
    ...
    [GeneratedRegex(@"^(\S+) connected", RegexOptions.Multiline)]
