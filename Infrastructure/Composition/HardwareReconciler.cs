...
/// <see cref="ReapplyTrigger"/>) and takes the axes, the order and the thread from <see cref="ReapplyPlan"/> —
/// Application's, because a set of actions is a use case.
///
/// WHY IT IS NOT IN APPLICATION, though the plan is. Executing the plan means calling this service and collecting
/// what the calling site's UI pass must show, and that result is built out of the persisted container's types
/// (<see cref="FanPreset"/>, <see cref="GpuOcPreset"/>), which are this layer's. Application may not name them,
/// and a contract that handed them over in a re-declared shape would be larger than the loop it serves — the
/// measurement is in the commit that split the two, and in <see cref="ReapplyPlan"/>'s own docstring.
///
...
internal sealed class HardwareReconciler
{
    ...
    /// <see cref="ReapplyPlan.Schedule"/> states them, and return the values the calling site's UI pass reflects.
    ///
    ...
    internal ReapplyOutcome Reapply(ReapplyTrigger trigger)
    {
        ...
        foreach (var axis in ReapplyPlan.Schedule(trigger))
        {
            ...
            if (ReapplyPlan.RunsOffTheCallersThread(trigger, axis))
            {
                ...
                if (axis == ModeAxis.Co && ReapplyPlan.Reflects(trigger)) outcome.Co = _svc.CurrentCoDomains();
                (deferred ??= []).Add(axis);
                continue;
            }
