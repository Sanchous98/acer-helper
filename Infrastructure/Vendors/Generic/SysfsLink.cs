...
// be the wrong trade. It uses nothing platform-specific (FileSystemInfo.LinkTarget exists on both TFMs).
//
// COMPILING FOR THE SUITE IS NOT THE SAME AS BEING REACHABLE BY IT, which is what the probes below are for: the
// walk cannot be handed a real tree, because its own spelling is POSIX (rooted at "/", split on '/'). Measured on
// the owner's machine, a directory named "AMDI0103:00" — one of the two handler nodes the token rule must tell
// apart — cannot exist on Windows at all (Directory.CreateDirectory answers IOException "Неверно задано имя
// папки."), and %TEMP% is spelled "C:\...", which a "/"-rooted path cannot name. So the two questions the walk
// asks of the filesystem arrive as delegates, and SysfsLinkTests drives the walk with a map of the real /sys tree.
...
/// resolved — the shape the VFS uses. The callers need the REAL path because the real path is what names the
/// driver: the class path says "platform-profile-1", the device path says
/// ".../acer-wmi/platform-profile/platform-profile-1", and the hwmon chip's device resolves to
...
internal static class SysfsLink
{
    ...
    /// <paramref name="names"/> answers "does this path name anything" once at the end.
    ///
    /// Null means it could not be resolved (a component that does not exist, a link loop, or nothing at the end).
    /// Callers treat null as "not this device": a node whose driver cannot be named is not one to bind a control
    /// to.</summary>
    internal static string? Walk(string path, Func<string, string?> linkTargetOf, Func<string, bool> names)
    {
        try
        {
            ...
            while (todo.Count > 0)
            {
                ...
                // relative to the link's CONTAINING directory — `resolved` without this last part, already fully
                // resolved, because we got here one component at a time.
                ...
                // context along with the target would add it a SECOND time.
    ...
    /// <see cref="LinkTargetOf"/> tries both: <c>Directory.Exists</c> is false for a path that names a FILE, so
    /// dropping the <c>File.Exists</c> half makes every file-naming path resolve to nothing.
    ///
    /// INTERNAL RATHER THAN PRIVATE so the suite can exercise it directly: this is the one place that asks the
    /// HOST, so no map of the kernel's tree can reach it (see SysfsLinkTests).</summary>
    ...
    /// so a single attempt would answer null for half the class tree. The ORDER matters: a class entry is a link
    /// to a DIRECTORY, and <c>FileInfo</c> alone answers null for one.
    ///
