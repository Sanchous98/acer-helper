...
// anything left in a Linux file cannot be reached by the suite at all. Everything that DECIDES lives here, and the
// two things that TOUCH the machine (the busctl call and the filesystem probe) arrive as delegates or from the
// per-OS host.
//
...
//     policy strings are case-sensitive; "Allow_dGPU" is the one runtime grant.
//   * Children inherit: the hook also accepts a pid whose REAL PARENT is in the allow-map.
...
// cardwire's SQLite app_policies table keyed by the LOWERCASE BINARY NAME — system state changed for every future
// run and for every other binary of the same name, on a key any process can spoof by renaming itself.
...
internal static class CardwireGpuAccess
{
    ...
    /// grants a running process access without writing anything down.</summary>
    ...
    /// for the discrete GPU without naming an index. (The daemon numbers them: on the AN18-61 the NVIDIA dGPU is
    /// cardwire's GPU 1, which is also the index its own log lines name.)</summary>
    ...
    /// SHARED between the row's refusal and the startup report.</summary>
    ...
    /// would lift, so on a machine that can already see the device the request is a no-op.</summary>
    ...
    ///
    /// THE BRANCHES: the choice is OFF (the only case in which the app touches nothing at all); not Linux (the
    /// capability does not exist here, whatever a copied settings file says); the GPU is already VISIBLE (the
    /// ordinary state of a machine in cardwire's Hybrid mode, not reported); there is no discrete GPU at all (same
    /// silence); and <see cref="ReportMissingService"/> — the one shut gate worth saying out loud, since a silent
    /// start would leave the user believing the app can see a GPU it cannot.
    ///
    /// NOTHING HERE RETRIES: the duty is consulted at startup and when the user flips the switch, never from the
    /// refresh loop.</summary>
    ...
    /// the machine stops hiding it, and a row that vanished with the offer would leave that choice with no switch to
    /// turn it off. What must not appear on a machine with nothing to ask for is the OFFER.</summary>
    internal static bool RowBelongs(in CardwireGpuAccessFacts facts, bool enabled)
        => enabled || ShouldOffer(facts);

    /// <summary>Why the gate is shut, in this port's own words. Null when there is something to ask for. The
    /// reasons are English, like every other transport's: the UI composes "«row label» failed: «reason»" from them
    /// and translates only the frame, because a daemon's refusal arrives in its own words.</summary>
    ...
    ///
    /// THE SIGNATURE IS PASSED EXPLICITLY ("usu"), a measured requirement: <c>busctl</c> infers a signature from
    /// bare arguments only for the simplest shapes, and the inferred call for this method is refused with "Too many
    /// parameters for signature" — verified on the owner's machine, where the inline form failed and the explicit
    /// one succeeded.
    ///
    /// THE PID IS THIS PROCESS'S: the grant is per-process and the daemon inserts exactly the pid it is given. It
    /// is a parameter rather than <c>Environment.ProcessId</c> read here so a test can hold the argument list to
    /// the pid it asked for.</summary>
...
/// because the hiding is exactly the difference between them: a hidden GPU is absent from /sys/bus/pci/devices
/// while /proc/bus/pci/devices still names it (the study's 44-vs-42 measurement). With one fact the app could not
/// tell "hidden" from "not there".
///
/// <para><see cref="IsLinux"/> is here for the same reason: the preference round-trips in a settings file, so a
/// run can hold it on a platform where the capability does not exist.</para></summary>
...
/// out to busctl, and this process's pid. Nothing here is static state, so a test drives the whole feature
/// without a daemon, a GPU or a filesystem.
///
/// THE REAL PORT IS BUILT BY THE OS (see <see cref="CardwireGpuAccessHost"/>): the portable TFM compiles exactly
/// one of <c>*.Linux.cs</c>/<c>*.Windows.cs</c>, so the file that names the machine is the file that may only
/// exist on one of them.</summary>
internal sealed class CardwireGpuAccessPort(Func<CardwireGpuAccessFacts> facts,
                                           Func<string[], (int code, string output)> busctl,
                                           Func<uint> ownPid)
{
    /// <summary>Whether the Options row belongs on screen. See <see cref="CardwireGpuAccess.RowBelongs"/>. Every
    /// reader probes AGAIN rather than caching the facts, because the world moves under the app: the two moments
    /// that matter (a UI being built, a click) are minutes apart.</summary>
    ...
    /// <summary>
    /// Ask cardwire to let THIS process see the discrete GPU — the one call this feature makes.
    ///
    /// IT RE-CHECKS THE GATE, so "nothing is asked for where there is nothing to ask for" holds at the moment of
    /// the call and not only at the moment the row was built.
    ///
    /// A FAILURE IS A RETURNED REASON, never an exception: the daemon may be gone, the bus may refuse the call (not
    /// in group wheel), or the interface may have changed under a newer cardwire — all three arrive as a non-zero
    /// exit with text, which the caller shows. Nothing is retried.
    ///
    /// THERE IS NO UN-DO: cardwire offers no revoke, so a successful call is in force until this process exits.
    /// The consent prompt says exactly that.</summary>
...
/// <c>CardwireGpuAccess.Windows.cs</c>) — the same shape as <c>MachineInfo</c>.
///
/// IT RETURNS A PORT ON EVERY PLATFORM RATHER THAN NULL, deliberately: on Windows there is no daemon to ask, and
/// that fact is one of the four the RULE takes (<see cref="CardwireGpuAccessFacts.IsLinux"/>), so the refusal
/// lives in the tested policy instead of being a second, untestable statement of the same rule in
/// composition.</summary>
